...
resource "aws_secretsmanager_secret_version" "app" {
  ...
  secret_string = jsonencode({
    ...
    # ANTHROPIC_API_KEY is operator-merged into the live JSON (LiteLLM only).
    # Terraform must not own the Anthropic console key. AEA_AI_ENDPOINT,
    # AEA_AI_API_KEY, and AEA_AI_MODEL stay operator-owned and are not
    # injected into orchestration until all three exist (follow-up).
  ...
  # later LITELLM_MASTER_KEY + AEA_AI_*). Applying this version blob would
  # drop those keys and revert a running BFF.
  lifecycle {
