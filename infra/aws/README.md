...
**This directory is IaC + operator docs.** GitLab `build-ecr` / `deploy-ecs`
jobs are a later MR. Do not `terraform apply` unless the operator explicitly
...
- **Path B (this stack, after a future apply):** RDS encryption at rest, ALB
  TLS via ACM, MSK TLS + SASL. Evidence is the applied cloud stack, not Compose.

## Prerequisites (needed before a future apply, not to merge this IaC)
