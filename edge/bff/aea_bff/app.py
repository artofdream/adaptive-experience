    ...
    def _least_data_operator_orders(raw: dict) -> dict:
        ...
        for item in (raw.get("items") or [])[:50]:
            ...
            if isinstance(timing, dict):
                safe_timing = {key: timing[key] for key in ("date", "window") if key in timing}
                if safe_timing:
                    shaped["timing"] = safe_timing
            if isinstance(shaped.get("card_message"), str):
                shaped["card_message"] = shaped["card_message"][:280]
