...
Background research notes and exploratory analyses that inform the AEA model
but are not yet promoted into `docs/`.
...
Stable research is promoted into `docs/` or `implementations/` via the loop SOP.
Canonical requirement IDs remain owned by `archive/` + published docs — not by
inbox notes.
