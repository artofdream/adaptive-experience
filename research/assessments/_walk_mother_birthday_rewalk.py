"""Mother-birthday rewalk against live https://localhost:8443/ (LiteLLM overlay)."""
...
OUT = HERE / "2026-08-15-2342-mother-birthday-e2e-rewalk.json"
SHOTS = HERE / "_walk_shots_rewalk"
...
report: dict = {
    ...
    "walked_at": "2026-08-15T23:42:00+02:00",
    "browser": "playwright (cursor-ide-browser MCP could not hold a tab)",
    "steps": [],
...
def main() -> int:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            report["notes"].append(f"chrome channel failed: {launch_err}")
            try:
        ...
        def send_and_wait(text: str, timeout_ms: int = 45000) -> dict:
            ...
            try:
                ...
                report["notes"].append(f"wait after send {text!r}: {type(exc).__name__}: {exc}")
        ...
        try:
            ...
            # If BFF timed out (503) while orchestration still accepted 202, wait for SSE.
            ...
            first_errors = [e for e in report["api"]["errors"] if e.get("url", "").endswith("/messages")]
            step(
                ...
                f"Notice: {after_partial.get('notice')!r}. Chips: {chips_after}. Customer line in log: {'I need flowers' in log}. FormError: {after_partial.get('formError')!r}. Messages: {after_partial.get('messages')}. First message API: {first_errors}. API suggestions: {report['api']['suggestions']}.",
            ...
            if need_full:
                # Avoid racing an in-flight first write (known stale_context).
            ...
            step(
                ...
                f"Intent panel: {intent_text[:500]!r}. Review and correct: {page.locator('#correct-open').is_visible()}. csrf_rejected={report['api']['csrf_rejected']}. dump={after_full}",
                "fail"
            ...
            if page.locator("#correct-open").is_visible() and any(
                ...
                # Wait until any in-flight update banner clears (known #184 if we don't).
                for _ in range(10):
                    txt = dump()
                    if "Updating" not in (txt.get("understanding") or "") and not txt.get("formError"):
                        break
                ...
                stale = "stale_context" in (intent_after + str(corr_err) + str(report["api"]["errors"])).lower()
                if stale:
                ...
                step(
                    ...
                    f"Saved recipient correction to 'Mum'. Panel now: {intent_after[:400]!r}. corr_ui={corr_err!r}. stale_context={stale}. errors={report['api']['errors']}",
                    "pass"
            ...
            slug_like = any(
                ("-" in c.split("|")[0] and c.split("|")[0].strip().islower())
                or "mixed-bunch" in c.lower()
                or "rose-dozen" in c.lower()
                for c in card_copy
            )
            usd = any("$" in c for c in card_copy)
            euro = any("€" in c for c in card_copy)
            report["notes"].append(
                f"T-03 names slug_like={slug_like} usd={usd} euro={euro} cards={card_copy}"
            )
            step(
