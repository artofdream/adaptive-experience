...
OUT = Path(__file__).with_name("2026-08-16-pilot-mother-birthday.json")
SHOTS = Path(__file__).with_name("_walk_shots_pilot")
    ...
    with sync_playwright() as p:
            ...
            if isinstance(body, dict):
                ...
                if rec["path"].endswith("/api/v1/session") and rec["method"] == "POST":
                    report["api"]["session_mint"] = {
                        "status": rec["status"],
                        "error": rec["error"],
                        "has_session": bool(body.get("session_id") or body.get("id")),
                        "correlation_id": body.get("correlation_id"),
                    }
        ...
        try:
            ...
            try:
                ...
                report["notes"].append(
                    "Stop: session/boot blocked. Do not apply migrations. DevSecOps owns bootstrap."
                )
                report["crash"] = actual
            ...
            if mint and mint.get("status") == 500:
                ...
                report["notes"].append(
                    "Stop: session mint 500. Do not apply migrations. Report to DevSecOps."
                )
