"""Mother-birthday customer walk against Path B after live-test inventory heartbeat.

URL: https://aea.artof.link/  (ACM TLS — do not ignore certificate errors)
Question: does T-03 Select enable after DSO in-VPC lily-reference-live-test
heartbeat (five SKUs every 30s)? Payment / T-07 Place Order skipped unless
checkout is clearly available without inventing card fields.
"""
...
OUT = Path(__file__).with_name("2026-08-17-mother-birthday-heartbeat.json")
SHOTS = Path(__file__).with_name("_walk_shots_heartbeat")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if card_count == 0:
                t03 = "fail"
                report["notes"].append(
                    "T-03 empty: no ranked cards after intent. "
                    "Heartbeat SKUs may not have reached recommendation ranking."
                )
