...
* **Layer 1 — Platform (zero-PII Engagement CRM, live):** `platform/aea_platform/crm.py` (`EngagementCrmService`) with schema `crm.customer_occasion_memory` (migration 018) persists only pseudonymous, non-PII attributes (`browser_hash`, `occasion_type`, `event_month/day`, `recipient_relation`). Reminders are **deterministic pull signals** computed on `GET /api/v1/crm/reminders` (proactive FCM/APNs push is not shipped — ADR-019); occasions are recorded via `POST /api/v1/crm/occasions`. Privacy lifecycle (NFR-017): customer **erasure** (`forget` / `DELETE /internal/v1/crm/occasions`) and time-based **retention** purge (`purge_expired`, ~13 months) via `platform/scripts/purge_crm_retention.py`. Delivered under M12; `FR-016`/`FR-017` remain **Future** in the source-of-truth workbook (reference-extension delivery).
