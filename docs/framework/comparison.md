...
4. **Antifragility** — when the same miss repeats, change the strongest layer (a sensor, a gate, a permission), not another paragraph of motivation. Related work calls this a ratchet. The rule is end to end: the framework site, the live florist, and how the work is run. AEA is **not** claiming to be antifragile yet. Dual-viewport is still a counter-example. A check that shipped as a build is another.
