...
>
> **Review wallet + Clear History (#410)** — **Code implemented (`versionCode` 9 / `0.1.0-alpha.9`); Play-honest device prove Unknown / pending.** App-bar **Privacy** opens the on-device list (recipient label, arrangement nickname, order ref, relative date). Confirm **Clear History** wipes `EdgeWallet`; Need Reorder disappears; Privacy shows the empty-state copy. JVM unit tests cover list + clear. Instrumentation (`EdgeWalletInstrumentationTest`) needs a device/emulator and is **not** a Play Internal prove. Do **not** mark this row Verified until sponsor ROG/cts-ai walks a Play-installed build after this lands.
>
...
Both v8 installs are signed and distributed by Google Play (`com.android.vending`) with no debug flag. Companion source on this change is `versionCode` **9** / `0.1.0-alpha.9` (#410). That bump is the next Play upload candidate — it is **not** a Play-install prove. This session did not inspect the sponsor evidence directories (`C:\Users\claud\Temp\aea-v8\evidence\`, `C:\Users\claud\Temp\aea-rog-404\`, `C:\Users\claud\Temp\aea-rog-reorder-2026-09-09\`).
...
3. **Review + Right to Be Forgotten (#410):** App-bar **Privacy** (not a cloud account) lists device-held receipts: recipient label, arrangement nickname, order ref, relative date — no street, no card PAN. **Clear History** confirms, then wipes the on-device wallet; Need Reorder disappears and Privacy shows an honest empty state. **Code on this change; Play-honest device prove Unknown / pending** until sponsor ROG/cts-ai can walk a Play Internal build. Do not treat review or Clear History as device **Verified** without that probe. Physical delivery addresses at the shop are still shredded after 14 days.
