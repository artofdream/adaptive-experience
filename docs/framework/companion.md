...
> **Installs from the Google Play Store** — **Verified (A36 and ROG Play Internal v8, 5 Sep 2026; ROG Play Internal v9, 9 Sep 2026):** Samsung A36 (`SM_A366B`, adb `RZCY60W1EZW`) and ASUS ROG (`ASUS_I001DC`, serial `K9AIKN07B088C89`) each installed companion `versionCode` **8** / `versionName` `0.1.0-alpha.8` from Play Internal (`installerPackageName=com.android.vending`, non-`DEBUGGABLE`). On 9 Sep 2026 ~21:40 Europe/Berlin the same ROG had Play Internal `versionCode` **9** / `0.1.0-alpha.9` (`com.android.vending`, non-`DEBUGGABLE`). Earlier Play-honest proves on ROG/A36 used prior Internal builds (v4–v7). A36 Play Internal v9 was **not** walked. **This page's 30s demo clip** remains the 4 Sep ROG App Dist / packageinstaller take — UX proof of Need→Pick→Pay→Confirmed, not Play-install honesty for that take.
>
...
>
> **Phone remembers the last order (Edge Wallet)** — **Write verified; Need reorder *code* on `main` (!459 / #404); Play Internal v8 tap Verified on ROG when a wallet receipt exists (9 Sep 2026):** After Confirm, the phone stores an encrypted receipt on the device (no street address, no card number). That **save** is a separate fact from the shortcut. Need shows a returning-customer card **only when a device-held receipt exists** (`latestWalletReceipt != null`). On 9 Sep 2026 ~00:10 Europe/Berlin, Play Internal v8 on ASUS ROG (`ASUS_I001DC`, serial `K9AIKN07B088C89`; package `link.artof.aea.companion` `versionCode` **8** / `0.1.0-alpha.8`; installer `com.android.vending`; non-`DEBUGGABLE`) opened Need with a wallet receipt already present. Reorder CTA **PRESENT**: "Reorder previous bouquet", "1-tap repeat order from this phone's encrypted wallet", button "Reorder →". Force-stop cold relaunch: still **PRESENT**. Tap Reorder → Pick with Classic Rose Dozen recommended (`content-desc` Classic Rose Dozen); Continue to Checkout enabled. On 5 Sep 2026, Play Internal v8 **fresh / empty-wallet Need** showed **no** Reorder CTA on **both** A36 and ROG (chips only). Empty-wallet **ABSENT** was **not** re-probed on 9 Sep (no `pm clear`). The #404 vault note records a sideloaded ROG walk — UX/code proof, not Play-install honesty. **This page's 30s demo clip** remains the !455 Need→Pick→Pay take and does not show the reorder card. Sponsor-machine ROG evidence path (not inspected from this checkout): `C:\Users\claud\Temp\aea-rog-reorder-2026-09-09\`.
>
> **Review wallet + Clear History (#410)** — **Verified (ROG Play Internal v9, 9 Sep 2026 ~21:40 Europe/Berlin).** App-bar **Privacy** opens the on-device list (recipient label, arrangement nickname, order ref, relative date). On ASUS ROG (`ASUS_I001DC`, serial `K9AIKN07B088C89`; package `link.artof.aea.companion` `versionCode` **9** / `0.1.0-alpha.9`; installer `com.android.vending`; non-`DEBUGGABLE`) Privacy was **PRESENT** and listed **2** receipts. Confirm **Clear History** wiped the wallet → empty state **"No receipts on this phone"**. Need Reorder CTA was **PRESENT** before clear and **ABSENT** after clear. JVM unit tests cover list + clear. Instrumentation (`EdgeWalletInstrumentationTest`) needs a device/emulator and is **not** a Play Internal prove. A36 Play Internal v9 was **not** walked. Sponsor-machine ROG evidence path (not inspected from this checkout): `C:\Users\claud\Temp\aea-rog-wallet-410-2026-09-09\`. #410 / !487 shipped the code; this row is the Play-honest device prove, not a reopen.
>
...
2. **1-Tap Reorder Shortcut (FR-008):** When a **local encrypted receipt** exists and Need is still fresh (no typed message, no occasion chip yet), a private card appears (*"Reorder for {recipient}"* or *"Reorder previous bouquet"*). Tapping **`Reorder →`** revalidates current cooler stock and takes you straight to Pick with your arrangement pre-selected. The card is **not** shown on an empty wallet. Play Internal v8 on ROG (9 Sep 2026) showed the card and walked the tap to Pick when a wallet receipt already existed. Play Internal v8 empty-wallet / fresh Need on A36 and ROG (5 Sep 2026) showed chips only. Empty-wallet **ABSENT** was not re-probed on 9 Sep (no `pm clear`).
3. **Review + Right to Be Forgotten (#410):** App-bar **Privacy** (not a cloud account) lists device-held receipts: recipient label, arrangement nickname, order ref, relative date — no street, no card PAN. **Clear History** confirms, then wipes the on-device wallet; Need Reorder disappears and Privacy shows **"No receipts on this phone"**. **Verified on ROG Play Internal v9** (9 Sep 2026 ~21:40 Europe/Berlin): Privacy listed 2 receipts; after confirm-clear the empty state appeared and Need Reorder was **ABSENT**. A36 Play Internal v9 was not walked. Physical delivery addresses at the shop are still shredded after 14 days.
