...
| **Layer 1: Reminder Outbox Dry-Run** | **Code on this slice (#428)** | Migration `028_crm_reminder_outbox.sql`. Upcoming occasion memory enqueues least-data `status=dry_run` / `not_sent` rows (categorical occasion/relation/days + AI or template copy). `GET /api/v1/operator/reminder-outbox` and `/florist` `#reminder-outbox` list counts without contact fields. Stub `POST .../send` returns `not_implemented` and never delivers. Parent #35 stays open for a live outbound channel. |
| **Layer 1: Operator Engagement Aggregates** | **Code on this slice (#421 / #427 / #430)** | `GET /api/v1/operator/engagement` and `/florist` `#engagement` show zero-PII counts: memory_count, unique_browsers, upcoming_within_days, subject_count, occasion / relation / event-month cohorts, and lifetime spend-band cohorts (`band_0_50` … `band_250_plus` from completed-order `subject_profile`). `GET /api/v1/operator/engagement/export?format=csv\|json` downloads the same allowlisted counts and categorical keys (fail-closed florist gate). No hashes, names, addresses, or subject list. Parent #36 stays open for ML and per-customer browse. |
...
| **Layer 2: Client-Side Edge Wallet** | **Save live / Need *code* on `main` (!459). Two Play facts (do not collapse them)** | Pure Kotlin on Android companion (ADR-020, FR-008 thin path; FR-008 stays Future in the workbook). Receipt write on Confirm probed. Need-screen one-tap reorder control is on `main` (#404) and shows only when a device-held receipt exists. **Empty-wallet Need** on Play Internal v8 (5 Sep 2026, ROG and A36) showed **no** CTA. **Receipt-present tap** on Play Internal v8 (9 Sep 2026, named ROG) is recorded on the [companion ledger](companion.html#verified-ledger). Empty-wallet **ABSENT** was not re-probed on 9 Sep. Device serials stay on that page. The #404 vault note is a sideload walk, not Play-install honesty. |
...
- No unsolicited push notifications, marketing emails, SMS blasts, or third-party ad retargeting pixels. Dry-run outbox (#428) is operator-visible and never delivers. A live outbound channel remains leftover on parent #35.
