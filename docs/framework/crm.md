...
| **Layer 1: Operator Engagement Aggregates** | **Code on this slice (#421 / #427)** | `GET /api/v1/operator/engagement` and `/florist` `#engagement` show zero-PII counts: memory_count, unique_browsers, upcoming_within_days, occasion / relation / event-month cohorts. `GET /api/v1/operator/engagement/export?format=csv\|json` downloads the same allowlisted counts and categorical keys (fail-closed florist gate). No hashes, names, or addresses. Parent #36 stays open for ML, spend-band dashboards, and per-customer browse. |
