...
[Home](index.html) · [Path B case study](path-b.html)
...
Named scripts used to probe the florist instantiation at [aea.artof.link](https://aea.artof.link). They are part of the schema so we can talk about the same thing. They are **not** proof that dual-viewport is done.

- **J1 Urgent Sam** — same-day roses, T-01 to T-04, no pay. A dual-viewport probe is this script on phone 9:16 and desktop 16:9 *after* the dual-viewport CSS. That clip is **Unknown**. **CF-054** is **regressed**.
- **J2 Planner Sarah** — T-04 card (satin ribbon). Dual CTA Update vs Continue was visible on 27 Aug clips. Those files are not in this repo.
- **J3 Loyal Alex** — reload; conversation persisted. No reorder badge (not faked).
- **J4 Tracker Chris** — “safe for cats?” Track locked; Contact Florist gated on tracking.

27 Aug clips were cut off-repo. They are evidence of that evening, not a post-CSS probe. This site does not host the video until a file is allowlisted here. Do not treat a screenshot gallery as verification.

## What this page is not

- Not the Path B shop, not DATE_RE, not internal vault papers, not a pitch deck, not 3DX Lab.
- Not AGENTS.md and not a 14/14 scoreboard.
- Not a request to restyle the florist.

Back to the [framework](index.html) or the [Path B case study](path-b.html).
