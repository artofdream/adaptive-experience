...
- **Review on this phone:** In the companion app bar, tap **Privacy** (not a Lily's Florist cloud account). The list shows only what this device holds: recipient label, arrangement nickname, order reference, and a relative date. Street address and card numbers are never stored in the Edge Wallet and do not appear here.
- **Instant Local Wipe:** Tapping **Clear History** asks you to confirm, then calls the on-device wipe (`EdgeWallet.clear()`). Encrypted receipts are deleted from this phone. The Need **Reorder** card disappears. The Privacy screen shows an honest empty state: no receipts on this phone; Need shows Reorder only after a later confirmed order. Lily's Florist has no cloud copy of this list to restore.
- **Play-honest device prove:** Code and JVM tests cover list + clear. A Play Internal walk on sponsor ROG / cts-ai is **pending / Unknown** until those handsets are available. This guide does not claim device **Verified** for review or Clear History.
