...
- [ ] 6. Collect one wait tag per stakeholder; tally throughput and cycle time
```
...
6. **Wait mix** — one tag per stakeholder (`merge` | `main` | `user` |
   `ownership` | `shop` | `no-assignment` | `none`) plus MRs merged since
   the last slot. Prefer specialist self-report; label PM observation if
   they have not answered this slot.
...
lane but cannot proceed. Do not mix the words.
...
| `none` | Actively working the named issue |
