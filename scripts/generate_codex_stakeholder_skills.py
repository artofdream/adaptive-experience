...
def main() -> None:
    ...
    parser.add_argument("--check", action="store_true",
                        help="fail if Cursor, Codex, Claude, Copilot, and Gemini skills differ")
    ...
    if drift:
        details = "\n - ".join(drift)
        raise SystemExit(
            "Cursor/Codex/Claude/Copilot/Gemini stakeholder skills are out of sync. Run "
            f"`python scripts/generate_codex_stakeholder_skills.py`.\n - {details}"
        )
    if args.check:
        print(f"ok: {len(SKILLS)} Cursor/Codex/Claude/Copilot/Gemini stakeholder skills are synchronized")
