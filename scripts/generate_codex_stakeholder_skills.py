...
GEMINI_TARGET = ROOT / ".gemini" / "skills"
GROK_TARGET = ROOT / ".grok" / "skills"
...
def render_gemini(name: str, description: str, linked: str, _adaptation: str) -> str:
    title = name.removeprefix("aea-").replace("-", " ")
    digest = canonical_digest(name)
    return f'''---
name: {name}
...
- Interpret `@aea-<role>` or `$aea-<role>` handoffs as the corresponding Gemini skill invocation (`{name}`).
...
def render_grok(name: str, description: str, linked: str, _adaptation: str) -> str:
    title = name.removeprefix("aea-").replace("-", " ")
    digest = canonical_digest(name)
    return f'''---
name: {name}
...
- Interpret `@aea-<role>` or `$aea-<role>` handoffs as the corresponding Grok skill invocation (`{name}`).
- Follow the repository-level instructions in `../../../AGENTS.md` and system rules.
...
def main() -> None:
    ...
    parser.add_argument("--check", action="store_true",
                        help="fail if Cursor, Codex, Claude, Copilot, Gemini, and Grok skills differ")
    ...
    for name, values in SKILLS.items():
        ...
        gemini_path = GEMINI_TARGET / name / "SKILL.md"
        gemini_expected = render_gemini(name, *values)
        if args.check:
            if (not gemini_path.is_file() or
                    gemini_path.read_text(encoding="utf-8") != gemini_expected):
                drift.append(str(gemini_path.relative_to(ROOT)))
        else:
            gemini_path.parent.mkdir(parents=True, exist_ok=True)
            gemini_path.write_text(gemini_expected, encoding="utf-8")

        grok_path = GROK_TARGET / name / "SKILL.md"
        grok_expected = render_grok(name, *values)
        if args.check:
            if (not grok_path.is_file() or
                    grok_path.read_text(encoding="utf-8") != grok_expected):
                drift.append(str(grok_path.relative_to(ROOT)))
        else:
            grok_path.parent.mkdir(parents=True, exist_ok=True)
            grok_path.write_text(grok_expected, encoding="utf-8")
    ...
    gemini_names = skill_names(GEMINI_TARGET)
    if gemini_names != configured_names:
        drift.append(
            f"Gemini inventory mismatch: missing={sorted(configured_names - gemini_names)}, "
            f"extra={sorted(gemini_names - configured_names)}"
        )
    grok_names = skill_names(GROK_TARGET)
    if grok_names != configured_names:
        drift.append(
            f"Grok inventory mismatch: missing={sorted(configured_names - grok_names)}, "
            f"extra={sorted(grok_names - configured_names)}"
        )
    if drift:
        details = "\n - ".join(drift)
        raise SystemExit(
            "Cursor/Codex/Claude/Copilot/Gemini/Grok stakeholder skills are out of sync. Run "
            f"`python scripts/generate_codex_stakeholder_skills.py`.\n - {details}"
        )
    if args.check:
        print(f"ok: {len(SKILLS)} Cursor/Codex/Claude/Copilot/Gemini/Grok stakeholder skills are synchronized")
